...
CREATE TABLE IF NOT EXISTS USER (
    email VARCHAR(128) NOT NULL, lname VARCHAR(64) NOT NULL, fname VARCHAR(64) NOT NULL, password VARCHAR(32) NOT NULL, phone_number VARCHAR(14), year VARCHAR(255), major VARCHAR(255), bio TEXT, budget DECIMAL(10, 2), personal_trait VARCHAR(255), PRIMARY KEY (email)
);

INSERT INTO
    USER (
        email, lname, fname, password, phone_number
    )
VALUES (
...
INSERT INTO
    USER (
        email, lname, fname, password, phone_number, year, major, bio, budget, personal_trait
    )
VALUES (
...
CREATE TABLE IF NOT EXISTS APT (
    email VARCHAR(128) NOT NULL, description VARCHAR(512), id VARCHAR(256) NOT NULL, PRIMARY KEY (id), CONSTRAINT fk_user_apt FOREIGN KEY (email) REFERENCES USER (email)
);

INSERT INTO
    APT (email, description, id)
VALUES (
...
CREATE TABLE IF NOT EXISTS ORGANIZATION (
    organization_id INT AUTO_INCREMENT, name VARCHAR(256) NOT NULL, email VARCHAR(128) NOT NULL, description VARCHAR(1024), owner_email VARCHAR(128) NOT NULL, category ENUM(
        'ACADEMIC', 'RECREATION', 'TECHNOLOGY', 'POLITICS', 'GREEKLIFE'
    ), member_count INT DEFAULT 1, PRIMARY KEY (organization_id), CONSTRAINT fk_user_organization FOREIGN KEY (owner_email) REFERENCES USER (email)
);

INSERT INTO
    ORGANIZATION (
        name, email, description, owner_email, category, member_count
    )
VALUES (
...
CREATE TABLE IF NOT EXISTS ORGANIZATION_ROSTER (
    roster_id INT AUTO_INCREMENT NOT NULL, user_email VARCHAR(128) NOT NULL, organization_id INT NOT NULL, type ENUM('MEMBER', 'MANAGER', 'OWNER') NOT NULL, PRIMARY KEY (roster_id), CONSTRAINT fk_user_manager FOREIGN KEY (user_email) REFERENCES USER (email), CONSTRAINT fk_organization_manager FOREIGN KEY (organization_id) REFERENCES ORGANIZATION (organization_id)
);

INSERT INTO
    ORGANIZATION_ROSTER (
        user_email, organization_id, type
    )
...
CREATE TABLE IF NOT EXISTS LOCATION (
    location_id INT AUTO_INCREMENT, location VARCHAR(256), organization_id INT NOT NULL, PRIMARY KEY (location_id), CONSTRAINT fk_organization_location FOREIGN KEY (organization_id) REFERENCES ORGANIZATION (organization_id) ON DELETE CASCADE
);

INSERT INTO
    LOCATION (location, organization_id)
VALUES ('Student Storage Unit 343', 2),
    ('Club Room Rack A', 2),
    ('Club Room Rack B', 2),
    ('Borrowed', 2);

CREATE TABLE IF NOT EXISTS ITEM (
    item_id INT AUTO_INCREMENT, name VARCHAR(128) NOT NULL, description VARCHAR(256), owner_email VARCHAR(128), quantity INT NOT NULL, category ENUM(
        'STATIONERY', 'MARKETING', 'ELECTRONICS', 'SUPPLIES', 'PERISHABLES', 'MERCHANDISE', 'TOOLS', 'CHEMICALS', 'FLAMMABLE', 'OTHER', 'UNIQUE', 'BOOKS'
    ), status ENUM('AVAILABLE', 'BORROWED') NOT NULL, location_id INT NOT NULL, organization_id INT NOT NULL, PRIMARY KEY (item_id), CONSTRAINT fk_location_item FOREIGN KEY (location_id) REFERENCES LOCATION (location_id), CONSTRAINT fk_organization_item FOREIGN KEY (organization_id) REFERENCES ORGANIZATION (organization_id) ON DELETE CASCADE
);

INSERT INTO
    ITEM (
        name, description, owner_email, quantity, category, status, location_id, organization_id
    )
VALUES (
...
CREATE TABLE IF NOT EXISTS REQUEST (
    request_id INT AUTO_INCREMENT NOT NULL, user_email VARCHAR(128) NOT NULL, organization_id INT NOT NULL, status ENUM(
        'PENDING', 'ACCEPTED', 'DECLINED'
    ) NOT NULL, description VARCHAR(256), type ENUM('JOIN', 'ITEM') NOT NULL, item_id INT, quantity INT, PRIMARY KEY (request_id), CONSTRAINT fk_user_request FOREIGN KEY (user_email) REFERENCES USER (email) ON DELETE CASCADE, CONSTRAINT fk_organization_request FOREIGN KEY (organization_id) REFERENCES ORGANIZATION (organization_id) ON DELETE CASCADE, CONSTRAINT fk_item_request FOREIGN KEY (item_id) REFERENCES ITEM (item_id)
);

INSERT INTO
    REQUEST (
        user_email, organization_id, status, description, type
    )
VALUES (
...
INSERT INTO
    REQUEST (
        user_email, organization_id, status, description, type, item_id, quantity
    )
VALUES (
...
CREATE TABLE IF NOT EXISTS LISTING (
    listing_id INT AUTO_INCREMENT, item_id INT NOT NULL, price DECIMAL(6, 2) NOT NULL, status ENUM('AVAILABLE', 'SOLD') NOT NULL, date_listed TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL, PRIMARY KEY (listing_id), CONSTRAINT fk_item_listing FOREIGN KEY (item_id) REFERENCES ITEM (item_id) ON DELETE CASCADE
);

CREATE TABLE IF NOT EXISTS FAVORITE (
    user_email VARCHAR(128) NOT NULL, listing_id INT NOT NULL, PRIMARY KEY (user_email, listing_id), CONSTRAINT fk_user_favorite FOREIGN KEY (user_email) REFERENCES USER (email), CONSTRAINT fk_listing_favorite FOREIGN KEY (listing_id) REFERENCES LISTING (listing_id)
);